...
pAlarmHandler : parent signal handler
 - 종료 : 일정 tick 도달시 child & parent kill
 - 자식 프로세서에 signal 보냄 
************************************************/
void pAlarmHandler(int signo){
	Pcb* next = NULL;
...
scheduler : select process
 - searchShort : cpu_time이 작은 프로세스 부터 실행
*****************************************************/
Pcb* scheduler(){
	//sjf
	Pcb* ret = NULL;
...
Pcb* searchShort(){
	...
	min = pcbs[0];
	for(i = 1; i <= 9; i++){
		if(pcbs[i]->remain_cpu_time < min->remain_cpu_time)
			min = pcbs[i];
	}
	return min;
}
